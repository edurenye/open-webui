<script lang="ts">
	...
	interface ImageConfig {
		...
		default_engine: 'openai' | 'gemini' | 'comfyui' | 'automatic1111';
		openai: {
			api_base_url: string;
			api_key: string;
	...
	const saveHandler = async () => {
		...
		const validConfig = {
			...
			gemini: {
				api_base_url: config.gemini?.api_base_url || '',
				api_key: config.gemini?.api_key || '',
				model_wrappers: (config.gemini?.model_wrappers || []).map(model_wrapper => ({
					id: model_wrapper.id,
					name: model_wrapper.name,
					model: model_wrapper.model,
					is_default: model_wrapper.is_default,
			...
			automatic1111: {
				api_base_url: config.automatic1111?.api_base_url || '',
				api_key: config.automatic1111?.api_key || '',
	...
	const addNewModelWrapper = async (engine: 'openai' | 'gemini' | 'comfyui' | 'automatic1111') => {
		if (!config) return;
		...
		switch (engine) {
			case 'openai':
				...
				break;
			case 'gemini':
				shouldBeDefault = !config.gemini.model_wrappers?.length;
				break;
			case 'automatic1111':
		...
		const defaultName = {
			...
			gemini: 'New Gemini model wrapper',
		...
		switch (engine) {
			case 'openai':
				...
				break;
			case 'gemini':
				if (!config.gemini.model_wrappers) config.gemini.model_wrappers = [];
				config.gemini.model_wrappers = [...config.gemini.model_wrappers, newModel];
				break;
			case 'automatic1111':
	...
	<div class="space-y-3 overflow-y-scroll scrollbar-hidden pr-2">
		{#if config}
			...
			<div class="space-y-4">
				...
				<div>
					<div class="mb-2 text-sm font-medium">{$i18n.t('Gemini Settings')}</div>
					<div class="flex gap-2 mb-4">
						<input
							class="flex-1 rounded-lg py-2 px-3 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
							placeholder={$i18n.t('API Base URL')}
							bind:value={config.gemini.api_base_url}
						/>
						<SensitiveInput
							placeholder={$i18n.t('API Key')}
							bind:value={config.gemini.api_key}
							required={false}
						/>
					</div>
					...
					<div class="space-y-4 mt-4">
						<div class="flex justify-between items-center">
							<div class="text-sm font-medium">{$i18n.t('Gemini Model Wrapper')}</div>
							<button
								type="button"
								class="px-3 py-1 text-sm bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 rounded-lg transition"
								on:click={() => addNewModelWrapper('gemini')}
							>
								{$i18n.t('Add model wrapper')}
							</button>
						</div>
						{#if config.gemini.model_wrappers?.length}
							{#each config.gemini.model_wrappers as model_wrapper, i}
								<div class="border dark:border-gray-850 rounded-lg p-4">
									<div class="flex justify-between mb-4">
										<div class="flex items-center gap-4">
											<h3 class="text-sm font-medium">{model_wrapper.name || 'New model wrapper'}</h3>
										...
										<button
											...
											on:click={() => {
												if (!config) return;
												config.gemini.model_wrappers = config.gemini.model_wrappers.filter((_, index) => index !== i);
											}}
										>
											<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
												<path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd" />
									...
									<div class="space-y-4">
										<div class="grid grid-cols-2 gap-4">
											<div>
												...
												<input
													...
													placeholder="e.g. SDXL 1.0"
													on:input={(e) => {
														model_wrapper.id = generateId(e.currentTarget.value || 'New Gemini model wrapper');
													}}
												/>
											</div>
											<div>
										...
										<div class="flex items-center space-x-2">
											<Switch
												...
												on:change={() => {
													if (model_wrapper.is_default) {
														config.gemini.model_wrappers.forEach((m, index) => {
															if (index !== i) m.is_default = false;
														});
													}
												}}
						...
						{:else}
							<div class="text-sm text-gray-500 dark:text-gray-400 text-center py-4">
								{$i18n.t('No Gemini model wrappers configured')}
							</div>
						{/if}
					</div>
				</div>
				...
				<div>
					...
					<div class="flex gap-2 mb-4">
						<input
							...
							bind:value={config.automatic1111.api_base_url}
						/>
						<SensitiveInput
							placeholder={$i18n.t('API Auth')}
							bind:value={config.automatic1111.api_key}
							required={false}
						/>
					</div>
				...
				<div>
					...
					<div class="flex gap-2 mb-4">
						<input
							class="flex-1 rounded-lg py-2 px-3 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
							placeholder={$i18n.t('Base URL')}
							bind:value={config.comfyui.api_base_url}
						/>
						<SensitiveInput
							placeholder={$i18n.t('API Key')}
										...
										<div class="space-y-4">
											<div>
												...
												<div class="flex w-full">
													<div class="flex-1">
														<input
															id="upload-comfyui-workflow-input-{i}"
															hidden
															type="file"
															accept=".json"
															on:change={(e) => {
																...
																reader.onload = (e) => {
																	model_wrapper.workflow = e.target.result;
																	e.target.value = null;
																};
														...
														<button
															...
															type="button"
															on:click={(e) => {
																document.getElementById(`upload-comfyui-workflow-input-${i}`)?.click();
															}}
														>
